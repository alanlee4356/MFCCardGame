...
IMPLEMENT_DYNAMIC(CCoupleHardDlg, CDialogEx)

CCoupleHardDlg::CCoupleHardDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_COUPLE_HARD_DIALOG, pParent)
	, m_nPoint1(0)
	, m_nPoint2(0)
	, m_nSecond(10)
{
...
BEGIN_MESSAGE_MAP(CCoupleHardDlg, CDialogEx)
	ON_WM_DESTROY()
	ON_WM_PAINT()
	ON_WM_TIMER()
	ON_WM_LBUTTONDOWN()
...
BOOL CCoupleHardDlg::OnInitDialog()
{
...
void CCoupleHardDlg::OnPaint()
{
	...
	else
	{
		...
		for (int i = 0; i < 64; i++)
		{
			...
			m_card_list[index].Draw(dc, 300 + (i % 8) * m_length, 50 + (i / 8) * m_height, m_length, m_height);
		}
...
void CCoupleHardDlg::OnTimer(UINT_PTR nIDEvent)
{
...
void CCoupleHardDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	...
	if (point.x > 300 && point.x < 300 + (m_length * 8) && point.x > 50 && point.y < 50 + (m_height * 8))
	{
...
HCURSOR CCoupleHardDlg::OnQueryDragIcon()
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
...
HBRUSH CCoupleHardDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CCoupleHardDlg::OnClickedButtonTimer()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	if (!m_bTimerRun)
	{
...
bool CCoupleHardDlg::IsGameComplete()
{
	// TODO: 여기에 구현 코드 추가.
	for (int i = 0; i < 64; i++) {
		if (m_game_table[i] != -1) {
